...
class ResourceRecommender:
    ...
    def train(self, X, y):
        ...
        grid_search = GridSearchCV(estimator=self.model, param_grid=param_grid, cv=3, scoring='neg_mean_squared_error')
        grid_search.fit(X, y)
...
def ensure_columns(df, columns):
    for column in columns:
        if column not in df.columns:
            df[column] = np.nan  # Use NaN to mark missing values
    return df
            ...
            if performance == "struggling":
                ...
                if filtered_resources:
                    X = [[resource['quality_score'], 
                          resource.get('clarity', 0),  # Ensure clarity is handled
                    ...
                    sorted_resources = [resource for _, resource in sorted(zip(predicted_scores, filtered_resources), reverse=True)]
